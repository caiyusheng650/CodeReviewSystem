...
const ApiKeyManager = () => {
  ...
  const fetchApiKeys = async () => {
    try {
      ...
      const sortedKeys = keysArray.sort((a, b) => b.is_active - a.is_active);
      ...
      // Ensure apiKeys is an array even on error
      setApiKeys([]);
    } finally {
      setLoading(false);
    }
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button
                          size="small"
                          variant="outlined"
                          onClick={() => handleToggleApiKeyStatus(apiKey._id, apiKey.status)}
                          disabled={loading}
                        >
